...
def FitGMM(Features_Folder, videolist):
    ...
    GMM_file=open("GMM_"+sys.argv[1]+".pkl",'wb')
...
def SVM_video_classify(root,SVM,GMM):
    ...
    if os.path.isfile(root+"/HOG.pkl") and os.path.isfile(root+"/HOG.pkl"):
        ...
        if n_traj>0:
            ...
            pick= np.sum(prediction == 'pick')
            drop= np.sum(prediction == 'drop')
            handshake= np.sum(prediction == 'handshake')
            nothing= np.sum(prediction == 'nothing')
...
def SVM_classify(GMM,Scaler,SVM,folder,list):


    bad_classify = open('bad_'+sys.argv[1]+"_"+sys.argv[2]+'.txt', 'w')
    ...
    for videofile in list:
        ...
        if root != "./Labeled/.DS_Store":
            ...
            classification=SVM_video_classify(root,Scaler,SVM,GMM)
            ...
            if label==classification:
                ...
                cuant+=1
                err+=1;
                #print(root,label,classification)
                line=root+'_'+str(classification)
                bad_classify.write("%s\n" % line)
    ...
    confussion_file=open("confussion_mtrx"+sys.argv[1]+"_"+sys.argv[2]+".pkl",'wb')
...
def main():
    ...
    SVM_file=open("SVM_"+sys.argv[1]+".pkl",'wb')
    ...
    SVM_classify(GMM,max_abs_scaler,SVM,Features_Folder,videolist)
